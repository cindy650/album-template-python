...
### 模板规格与页数判定（2026-09-16）

产品通过 `template_marker` 唯一匹配模板后，读取 `specification_field` 对应的订单值：

- 能在该模板规格子表中命中：按匹配规格处理。
- 字段有值但未命中规格：按业务约定保留为页数信息，使用该模板默认规格，不能清空已匹配的模板 ID。例如 `Quantity of sheets = 40 sheets` 不应阻止模板关联；原始字段保留供后续页数和背脊计算使用。
- 未配置 `specification_field`：使用模板默认规格。
- 配置了字段但订单缺失该值：仍保持未关联，不把缺失值当作页数。

该规则适用于已有商品名、`product_identifiers` 和 DeepSeek 分类后的模板匹配。
只有模板名称唯一命中才允许上述默认规格处理，不能用页数回退跨模板猜测。
...
商品分类阶段在 `MailProductResolver` 中得到的 `size_template_id` 会显式写入当前商品项，再传给 `upsert()`；入库阶段优先保留该结果，不再用旧的固定字段匹配器覆盖。入库仍会校验店铺、产品和模板归属。
...
3. 按页数和模板配置计算背脊宽度等渲染参数。模板级页数背脊范围先按模板/显示单位解释，再转换到选中规格的 `size_unit`；例如 `1.397 cm` 在英寸规格中必须保存为 `0.55 in`，不能直接当成 `1.397 in`。
...
9. 预览图发送成功后写入 `orders.wecom_preview_sent=1`，但不修改订单状态；前端单独调用 `POST /api/v1/orders/status/advance`，按状态表顺序推进，支持 `0` → `1`。

生产单（A4）会从订单 `product_information.product_image` 读取商品缩略图；
图片成功加载后按比例缩放并放在商品信息栏左侧，同时绘制边框。该字段不会作为
商品文字或翻译内容输出；图片地址失效时记录 `[生产单] 商品图片加载失败`，其余文字仍照常生成。
