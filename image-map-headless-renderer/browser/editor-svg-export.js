"use strict";
var ImageMapEditorSvgExportBundle = (() => {
  ...
  // node_modules/text-to-svg/node_modules/opentype.js/dist/opentype.js
  var require_opentype = __commonJS({
    "node_modules/text-to-svg/node_modules/opentype.js/dist/opentype.js"(exports, module) {
  ...
  var require_src = __commonJS({
    "node_modules/text-to-svg/build/src/index.js"(exports, module) {
      "use strict";
      Object.defineProperty(exports, "__esModule", {
        value: true
      });
      exports.default = void 0;
      var opentype = _interopRequireWildcard(require_opentype());
      ...
      var TextToSVG2 = /* @__PURE__ */ function() {
        function TextToSVG3(font) {
          _classCallCheck(this, TextToSVG3);
          this.font = font;
        }
        _createClass(TextToSVG3, [{
          key: "getWidth",
          ...
          value: function loadSync() {
            ...
            return new TextToSVG3(opentype.loadSync(file));
          }
        }, {
          key: "load",
          value: function load(url, cb) {
            opentype.load(url, function(err, font) {
              ...
              return cb(null, new TextToSVG3(font));
            });
          }
        }]);
        return TextToSVG3;
      }();
      exports.default = TextToSVG2;
  ...
  var require_text_to_svg = __commonJS({
    "node_modules/text-to-svg/index.js"(exports, module) {
      module.exports = require_src();
    }
  });

  // src/image-map-editor/canvas/utils/exportCorelCompatibleSvg.ts
  var SVG_NAMESPACE = "http://www.w3.org/2000/svg";
  var XLinkNamespace = "http://www.w3.org/1999/xlink";
  var compactStyleProperties = /* @__PURE__ */ new Set([
    "fill",
    "fill-rule",
    "fill-opacity",
    "stroke",
    ...
    Array.from(root.querySelectorAll("*")).forEach((element) => {
      ...
      if (element.getAttribute("style")) {
        const declarations = new Map(
          (element.getAttribute("style") || "").split(";").map((rule) => rule.trim()).filter(Boolean).map((rule) => {
        ...
        if (declarations.get("stroke-opacity") === "0" || declarations.get("stroke") === "none") {
          [
            ...
            "stroke-dasharray",
            "stroke-dashoffset"
          ].forEach((property) => declarations.delete(property));
        }
        const compactStyle = Array.from(declarations.entries()).filter(([property]) => compactStyleProperties.has(property)).map(([property, value]) => `${property}:${value}`).join(";");
  ...
  var addBackground = (document2, root, bounds, color) => {
    const background = document2.createElementNS(SVG_NAMESPACE, "rect");
    ...
    const firstLayer = Array.from(root.children).find(
      (element) => element.localName !== "defs" && element.localName !== "style" && element.localName !== "title"
    );
    root.insertBefore(background, firstLayer || null);
  };
  var addExternalFonts = (document2, root, fontSources) => {
    const fonts = (fontSources || []).filter(
      (font) => font && font.family && font.url && !/^(data:|blob:)/i.test(font.url)
    );
    if (!fonts.length) return;
    const style = document2.createElementNS(SVG_NAMESPACE, "style");
  ...
  var addLayerNames = (root, layerNames) => {
  ...
  var translateContentToWorkarea = (root, bounds) => {
  ...
  var flattenGroups = (root) => {
    const flatten = (group) => {
      Array.from(group.children).forEach((child2) => {
        if (child2.localName === "g") flatten(child2);
      });
    ...
    Array.from(root.children).forEach((element) => {
      if (element.localName === "g") flatten(element);
    });
  };
  var splitFabricTextLines = (document2, root) => {
    Array.from(root.querySelectorAll("text")).forEach((textElement) => {
      ...
      tspans.forEach((tspan) => {
        const glyph = document2.createElementNS(SVG_NAMESPACE, "text");
  ...
  var addPrintGuides = (document2, root, bounds, printGuides) => {
    (printGuides || []).forEach((guide, index) => {
      const line = document2.createElementNS(SVG_NAMESPACE, "line");
  ...
  var exportCorelCompatibleSvg = ({
    ...
    const document2 = new DOMParser().parseFromString(rawSvg, "image/svg+xml");
    const root = document2.documentElement;
    if (document2.querySelector("parsererror") || root.localName !== "svg") throw new Error("Fabric \u5BFC\u51FA\u7684 SVG XML \u65E0\u6548");
    removeFabricNoise(root);
    addBackground(document2, root, bounds, backgroundColor);
    addLayerNames(root, layerNames);
    translateContentToWorkarea(root, bounds);
    splitFabricTextLines(document2, root);
    flattenGroups(root);
    addExternalFonts(document2, root, fontSources);
    addPrintGuides(document2, root, bounds, printGuides);
  ...
  // src/image-map-editor/canvas/utils/exportTextToSvg.ts
  var import_text_to_svg = __toESM(require_text_to_svg(), 1);

  // src/image-map-editor/canvas/utils/pathTextUtils.ts
  var buildWhitespaceSafePathRuns = (text, x, y, metrics) => {
    const runs = [];
    let cursorX = x;
    let segment = "";
    const flush = () => {
  ...
  var normalizeFamily = (value) => value.split(",")[0].trim().replace(/^['"]|['"]$/g, "").toLowerCase();
  var styleValue = (element, property) => {
    const value = element.getAttribute(property);
    if (value) return value;
    const declaration = (element.getAttribute("style") || "").split(";").find((item) => item.split(":")[0]?.trim().toLowerCase() === property.toLowerCase());
  ...
  var loadFont = (url) => {
    ...
    const promise = new Promise((resolve, reject) => {
      import_text_to_svg.default.load(url, (error, textToSvg) => {
        if (error || !textToSvg) {
  ...
  var copyPathAttributes = (text, path) => {
  ...
  // image-map-headless-renderer/browser/editor-svg-export-entry.ts
